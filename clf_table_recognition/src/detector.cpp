  ...
  :  nh_(nh), tf_listener(tf_buffer)
{
  ...
  srv_table_ = nh.advertiseService("is_door_open", &Detector::ServiceIsDoorOpen, this);
...
bool Detector::ServiceIsDoorOpen(clf_object_recognition_msgs::Door::Request& req,
    clf_object_recognition_msgs::Door::Response& res)
{
    ...
    pcl::PointCloud<point_t>::Ptr cloud_filtered(new pcl::PointCloud<point_t>);
    {
        ...
        pcl::CropBox<point_t> cropBoxFilter (true);
        cropBoxFilter.setInputCloud (cloud_voxel);
        Eigen::Vector4f min_pt (req.min_box_point.x, req.min_box_point.y, req.min_box_point.z, 1.0f);
        Eigen::Vector4f max_pt (req.max_box_point.x, req.max_box_point.y, req.max_box_point.z, 1.0f);
    ...
    ROS_DEBUG_STREAM_NAMED("detector", "Number of points after filtering: " << cloud_filtered->size());
    return true;
}