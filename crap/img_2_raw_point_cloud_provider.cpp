                                                  ...
                                                  clf_object_recognition_msgs::Img2RawPointCloudMsg::Response& res)
{
  ...
  bool is_bbox_specified =
      (req.bbox.center.x != 0 || req.bbox.center.y != 0 || req.bbox.size_x != 0 || req.bbox.size_y != 0);
  ...
  // The crop region is defined in the camera frame, but the resulting point cloud needs to be transformed to the fixed
  // frame
  ...
  try
  {
    transformStamped_c = tf_buffer.lookupTransform(std::string(req.camera_link),  // cast to string
  ...
  crop_transform.setOrigin(tf::Vector3(xmin * info_msg.K[0] + info_msg.K[2], ymin * info_msg.K[4] + info_msg.K[5], 0));
...
int main(int argc, char** argv)
{
  ...
  ros::ServiceServer service =
      nh.advertiseService("img_2_raw_point_cloud", pointcloud_from_depth_image_service_callback);
