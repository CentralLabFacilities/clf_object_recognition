...
object_msgs::Detection3D pcl_to_detection(const sensor_msgs::PointCloud2& pcl_msg, const std::string& class_name, const float& score)
{
    object_msgs::Detection3D detection;
    ...
    object_msgs::ObjectHypothesis hypothesis;
    hypothesis.class_name = class_name;
    ...
    detection.bounding_box.size = size;
...
bool detection_service(object_msgs::DetectObject::Request& req,
                       object_msgs::DetectObject::Response& res)
{
    object_msgs::Detection3D detection = pcl_to_detection(req.pcl, req.class_name, req.score);
